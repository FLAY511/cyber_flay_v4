...
osint_whois() {
  ...
  if need_cmd whois whois; then whois "$domain" | head -n 80; fi
  echo -e "${cyan}Alternatif RDAP:${nc}"; curl -s "https://rdap.org/domain/$domain" | jq . 2>/dev/null || true
  pause
}

osint_geoip() {
  read -p "IP / domain: " target
  curl -s "http://ip-api.com/json/$target" | jq . 2>/dev/null || curl -s "http://ip-api.com/json/$target"
  pause
}

osint_dns_lookup() {
  read -p "Domain: " d
  if need_cmd dig dnsutils; then
    dig +short A "$d"
    dig +short MX "$d"
    dig +short NS "$d"
  fi
...
osint_reverse_dns() {
  read -p "IP: " ip
  if need_cmd dig dnsutils; then dig -x "$ip" +short; fi
  pause
}

osint_subdomain_crt() {
  read -p "Domain: " d
  curl -s "https://crt.sh/?q=%25.$d&output=json" | jq -r '.[].name_value' 2>/dev/null | sort -u | sed 's/\*\.//g' | sed 's/\r//g'
  pause
}
...
osint_traceroute() {
  read -p "Domain/IP: " t
  if need_cmd traceroute traceroute; then traceroute "$t"; fi
  pause
}
...
osint_portscan() {
  ...
  read -p "Port list (misal: 21,22,80,443,8080): " plist
  IFS=',' read -ra ports <<< "$plist"
  if need_cmd nc netcat-openbsd; then
    for p in "${ports[@]}"; do
      echo -ne "Port $p: "
      (nc -zv -w 2 "$t" "$p" >/dev/null 2>&1 && echo "OPEN") || echo "CLOSED"
    done
  fi
  pause
}
...
osint_ssl_info() {
  read -p "Domain (tanpa https): " d
  if need_cmd openssl openssl; then
    echo | openssl s_client -showcerts -servername "$d" -connect "$d:443" 2>/dev/null | openssl x509 -noout -issuer -subject -dates -fingerprint -sha256
  fi
  pause
}

osint_reverse_ip_domains() {
  read -p "IP: " ip
  echo "[Hackertarget] (rate limit)"
  curl -s "https://api.hackertarget.com/reverseiplookup/?q=$ip"
  pause
}

osint_email_breach() {
  echo "[PERINGATAN] Endpoint HIBP memerlukan API key berbayar."
  echo "Set ENV: export HIBP_KEY=YOUR_KEY"
  read -p "Email: " email
  if [ -n "$HIBP_KEY" ]; then
    curl -s -H "hibp-api-key: $HIBP_KEY" -H "user-agent: CYBER-FLAY" "https://haveibeenpwned.com/api/v3/breachedaccount/$email?truncateResponse=false" | jq . 2>/dev/null || echo "Tidak ada/terblokir."
  else
    echo "HIBP_KEY belum di-set. Lewati."
  fi
  pause
}
...
osint_github_user() {
  ...
  curl -s "https://api.github.com/users/$u" | jq . 2>/dev/null || curl -s "https://api.github.com/users/$u"
  pause
}

osint_username_check() {
  read -p "Username: " u
  echo "Cek eksistensi username di beberapa platform:"
  for base in "https://github.com" "https://gitlab.com" "https://www.reddit.com/user"; do
    url="$base/$u"
...
menu_osint() {
  while true; do
    ...
    echo "15) Username Check (GitHub/GitLab/Reddit)"
    echo " 0) Kembali"
    echo "========================================"
    read -p "Pilih: " c
    case "$c" in
...
menu_lain() {
  while true; do
    clear; hacker_ascii
    echo -e "${yellow}=========== MENU LAIN (10 Fitur) =======${nc}"
...
while true; do
  ...
  echo -e "${yellow}3.${nc} Menu Lain (10 fitur)"
